/*global $, window*/
    ...
    function showdiv(d){
        ...
        document.getElementById(d + 'iframe').style.display = 'block';
        if (cur_div != d) {
            if (cur_div != 'no') {
...
$.fn.editableTableWidget = function (options) {
	...
	return $(this).each(function () {
			...
			showEditor = function (select, keyInput) {
				...
				if (active.length) {
					...
					if(header.hasClass('autocomplete')){
						...
						if(tableHeader){
							var src = tableHeader.find("th:eq("+active.index()+")").attr("select-source");
							editor.autocomplete({
								source: src ? JSON.parse( src ) : [],
								minLength: 1
								/*
		...
		element.on('keypress dblclick', function(e){
			...
			if(e.type == "keypress"){
				if(thCurrent.hasClass('editor-cancel')){
					return;
				}

				if(thCurrent.hasClass('data-type-address') || thCurrent.hasClass('data-type-shipping-address')){
       				return;
				}
			...
			if(editorID != null && editorID[1]){
				...
				if (editorElem == "stock_type"){

				}
				else{
					if (editorElem == "cancel"){
						return;
					}
					else if ($('#'+editorElem).length == 0){
						return;
					}
					else{
						editTag = editorElem;
