...
var AVIMGlobalConfig = {
	...
	controlCSS: "bms/common/avim.css" //Path to avim.css
};
...
function AVIM()	{
	...
	if(AVIMGlobalConfig.showControl) {
		...
		document.write('<input id="avim_auto" type="radio" name="AVIMMethod" onclick="AVIMObj.setMethod(0);" />Tự động (F9)');
...
//		document.write('<input id="avim_viqr2" type="radio" name="AVIMMethod" onclick="AVIMObj.setMethod(4);" />VIQR*');
		document.write('<input id="avim_off" type="radio" name="AVIMMethod" onclick="AVIMObj.setMethod(-1);" />Tắt (F9)');
...
//		document.write('<input type="checkbox" id="avim_daucu" onclick="AVIMObj.setDauCu(this);" />Kiểu cũ</p>');
		document.write('&nbsp;&nbsp;<a class="AVIMControl" onclick="document.getElementById(' + "'AVIMControl').style.display='none';" + '">[Ẩn - F12]</a>');
		document.write('</div>');
	}
	...
	this.keyDownHandler = function(e) {
		if(e == "iframe") {
			...
			var key = (! e.which) ? window.event.keyCode : e.which;
		}
		if (key == 120) {
...
//document.getElementById('AVIMControl').style.left=(screen.width-370)+"px";
