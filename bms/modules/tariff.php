    ...
    function child_run() {
        ...
        if ($sessions->get_session("login") != "logined" && $action->eda_code != md5("login") && $action->eda_code != md5("in") && $action->eda_code != md5("reg") && $action->eda_code != md5("forgot") && $transfer == false) {
        	...
        	switch ($action->eda_code) {
                ...
                case md5('load_stock_type_data_for_tarriff'):
                    $warehouse_id       = $_POST['warehouse_id'];
                    $stock_type_name    = $_POST['stock_type_name'];

                    $query_data = get_data("select * from stock_type where warehouse_id='".$warehouse_id."' and stock_type_name='".$stock_type_name."' and CHAR_LENGTH('".$stock_type_name."')=CHAR_LENGTH('".$stock_type_name."')");
                    $data = array();
                ...
                case md5('load_region_for_tariff'):
                    $s_nation           = $_POST['s_nation'];
                    $s_city             = $_POST['s_city'];
                    $r_nation           = $_POST['r_nation'];
                    $r_city             = $_POST['r_city'];

                    $s_query_data = get_data("select * from warehouse where nation_id='".$s_nation."' and city_id='".$s_city."'");
                    $r_query_data = get_data("select * from warehouse where nation_id='".$r_nation."' and city_id='".$r_city."'");
                    $t_query_data = get_data("select * from trf_transport where warehouse_id_dep='".$s_query_data[0]['warehouse_id']."' and warehouse_id_des='".$r_query_data[0]['warehouse_id']."'");

                    $s_data = array();
                    $r_data = array();
                    $t_data = array();
                    
                    for ($i = 0; $i < count($s_query_data); $i++){
                         $s_data[] = $s_query_data[$i];
                    }

                    for ($i = 0; $i < count($r_query_data); $i++){
                         $r_data[] = $s_query_data[$i];
                    }

                    for ($i = 0; $i < count($t_query_data); $i++){
                         $t_data[] = $t_query_data[$i];
                    }

                    $data = array($s_data, $r_data, $t_data);
                    echo(json_encode($data, false));
                    exit();
                    break;

                case md5('load_tariff_data_for_tariff_confirm'):
                    $query_type     = $_POST['query_type'];
                    $query_data     = get_data("select * from ord_tariff where status='KT' or status='NK'");
                    $query_data_KT  = get_data("select * from ord_tariff where status='KT'");
                    $query_data_NK  = get_data("select * from ord_tariff where status='NK'");
                    ...
                    if ($query_type == 'all'){
                        $dataPush = array();
                        foreach($query_data as $item => $value){
                            if ($value['status'] == 'KT'){
                                $status_btn = '<button style="font-size: 11px; background-color: transparent; margin-right: 3px; color: red; font-weight: bold; border: none;" value="Duyệt" class="btnConfirm" attrX="'.$value['ord_trf_id'].'"><u>Duyệt</u></button>';
                            }
                            else{
                                $status_btn = '<label style="font-size: 11px; background-color: transparent; margin-right: 3px; color: green; font-weight: bold; border: none;">Đã nhập kho</label><button style="font-size: 11px; background-color: transparent; margin-right: 3px; color: red; font-weight: bold; border: none;" value="Hoàn tác" class="btnReturn" attrX="'.$value['ord_trf_id'].'"><u>(Hoàn tác)</u></button>';
                            }

                            $dataPush = array($index++, $value['ord_trf_id'], $value['ord_trf_code'], $status_btn, $value['sender_id'], $value['sender_name'], $value['sender_tel'], $value['sender_email'], $value['sender_address'], $value['receiver_name'], $value['receiver_tel'], $value['receiver_email'], $value['receiver_address'], $value['warehouse_id_dep'], $value['warehouse_code_dep'], $value['warehouse_name_dep'], $value['warehouse_id_des'], $value['warehouse_code_des'], $value['warehouse_name_des'], $value['nation_id'], $value['nation_code'], $value['nation_name'], $value['city_id'], $value['city_code'], $value['city_name'], $value['remark'], $value['status']);

                            $data[] = $dataPush;
                        } 
                    }
                    else if ($query_type == 'KT'){
                        $dataPush = array();
                        foreach($query_data_KT as $item => $value){
                            if ($value['status'] == 'KT'){
                                $status_btn = '<button style="font-size: 11px; background-color: transparent; margin-right: 3px; color: red; font-weight: bold; border: none;" value="Duyệt" class="btnConfirm" attrX="'.$value['ord_trf_id'].'"><u>Duyệt</u></button>';
                            }
                            else{
                    ...
                    else if ($query_type == 'NK'){
                        $dataPush = array();
                        foreach($query_data_NK as $item => $value){
                            print_r('xxx');
                            if ($value['status'] == 'KT'){
                                $status_btn = '<button style="font-size: 11px; background-color: transparent; margin-right: 3px; color: red; font-weight: bold; border: none;" value="Duyệt" class="btnConfirm" attrX="'.$value['ord_trf_id'].'"><u>Duyệt</u></button>';
                            }
                ...
                case md5('load_tariff_details_data_for_tariff_confirm'):
                    $ord_trf_code = $_POST['ord_trf_code'];
                    $query_data = get_data("select * from ord_tariff_details where ord_trf_code='".$ord_trf_code."'");
                    $index = 1;
                    $final_total_money = 0;
                    $data = array();

                    foreach($query_data as $item => $value){
                        $dataPush = array($index++, $value['ord_trf_details_id'], $value['ord_trf_code'], $value['stock_type_id'], $value['stock_type_code'], $value['stock_type_name'], $value['stock_type_details'], $value['numeric'], $value['stock_weight'], $value['unit_id'], $value['unit_code'], $value['unit_name'], $value['price'], $value['expense_type'], $value['currency_id'], $value['currency_code'], $value['currency_name'], $value['rates'], $value['vat_rates'], $value['expense'], $value['total_money'], $value['remark']);
                        $final_total_money += $value['total_money'];
                        $data[] = $dataPush;
                    }
                    $data = array($data, $final_total_money);
                    echo(json_encode($data, false));
                    exit();
                    break;
...
//print_r($data); die('xxx');
    ...
    public function add_contract_details($data){
        ...
        if(is_array($data) && count($data)>0){
            crm_add_contract($data[0]['contract_code']);
        }
    }
    ...
    function update_ORD_TRF_on_shipping($data){
        ...
        for ($i = 0; $i < count($data); $i++){
            ...
            crm_update_ord_status($data[$i] ['status'],$data [$i]['ord_trf_code'],0);
    ...
    function update_ORD_TRF_on_release($data, $ordIDList){
        ...
        for ($i = 0; $i < count($data); $i++){
            ...
            if(is_array($dt) && count($dt)>0){
                ...
                foreach ($dt as $kk => $vl) {
                    ...
                    crm_update_ord_status($data[$i]['status'],$vl['ord_trf_code'],0);
                }
        ...
        foreach ($ddt as $kk => $vl) {
            crm_update_ord_status($vl['status'],$vl['ord_trf_code'],0);
        }
    }
    ...
    function update_transported_internal_warehouse_data($data){
        ...
        for ($i = 0; $i < count($data); $i++){
            $db->query("update ord_tariff set trs_internal_warehouse_code='" . $data [$i]['trs_internal_warehouse_code'] 
                        ...
                        . "', update_time='"                    . $data [$i]['update_time'] 
                        . "' where ord_trf_code ='"             . $data [$i]['ord_trf_code'] . "'");
            crm_update_ord_status($data[$i] ['status'],'',$data[$i] ['ord_trf_id']);
            $this->send_mail($data[$i]['ord_trf_id']);
        }
    ...
    function update_status_ord_tariff_details_data($data){
        ...
        $db->query("update ord_tariff set status='" . $data[0] ['status']
                ...
                . "' where ord_trf_id='" . $data[0] ['ord_trf_id'] . "'");
        crm_update_ord_status($data[0] ['status'],'',$data[0] ['ord_trf_id']);
    ...
    function add_ord_tariff_details_data($data){
        ...
        if(count($data)>0){
            ...
            if(is_array($checkItem) && count($checkItem)>0){
               $pj_id= crm_add_project($checkItem[0]['ord_trf_code'],$checkItem[0]['sender_email'],$checkItem[0]['receiver_email'],$tongtien,$details);
