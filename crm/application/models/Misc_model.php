...
class Misc_model extends App_Model
{
    ...
    public function get_user_notifications($read = false)
    {
        ...
        $staffx=$this->db->select('group_concat(departmentid) deplist')->from(db_prefix() . 'staff_departments')->where('staffid', $staff_id)->get()->row_array();
        $deplist=@$staffx['deplist'];
        $where='touserid=' . $staff_id;
        if($deplist!='')
        $where.=' or departmentid in ('.$deplist.')';
        $where='('.$where.')';
        $sql = 'SELECT COUNT(*) as total FROM ' . db_prefix() . 'notifications WHERE isread=' . $read .' and '. $where;
        $sql .= ' UNION ALL ';
        $sql .= 'SELECT COUNT(*) as total FROM ' . db_prefix() . 'notifications WHERE isread_inline=' . $read .' and '. $where;
