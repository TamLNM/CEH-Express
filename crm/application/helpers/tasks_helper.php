...
function tasks_summary_data($rel_id = null, $rel_type = null)
{
    ...
    foreach ($statuses as $status) {
        ...
        $tasks_my_where = '(id IN(SELECT taskid FROM ' . db_prefix() . 'task_assigned WHERE staffid=' . get_staff_user_id() . ') OR ' . db_prefix() . 'tasks.departmentid in (select departmentid from ' . db_prefix() . 'staff_departments where staffid='.get_staff_user_id().')) AND status=' . $status['id'];
        if ($rel_id && $rel_type) {
...
{
    $_tasks_where = '('.db_prefix().'tasks.id IN (SELECT taskid FROM '.db_prefix().'task_assigned WHERE staffid = ' . get_staff_user_id() . ') OR ' . db_prefix() . 'tasks.departmentid in (select departmentid from ' . db_prefix() . 'staff_departments where staffid='.get_staff_user_id().') OR '.db_prefix().'tasks.id IN (SELECT taskid FROM '.db_prefix().'task_followers WHERE staffid = ' . get_staff_user_id() . ') OR (addedfrom=' . get_staff_user_id() . ' AND is_added_from_contact=0)';
