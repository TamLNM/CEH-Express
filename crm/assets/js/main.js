...
function numF(value){
    ...
    var rg3 = /(?!\.)\D/g;
    var rg4 = /\B(?=(\d{3})+(?!\d))/g;
    var rt = value.replace(rg1, "").replace(rg2, "").replace(rg3, "").replace(rg4, ",").split('.');
    console.log(rt);
    if(typeof rt[1]=="undefined" || rt[1]=='00')
        return rt[0];
    else
        return rt[0]+"."+(rt[1].length>2?rt[1].substring(0, 2):rt[1]);
}
$(function() {



var jgi=$("input[name='amount'],input[name='rate'],input[name='contract_value'],input[name='items[][\"rate\"]']");
if(jgi.length>0){
    jgi.each(function(){
        $(this).addClass("price_input").attr("type","text");
    });
}
$(".price_input").each(function(){
    if($(this).val()!=numF($(this).val())){
        $(this).val(numF($(this).val()));
    }
});
$(document).on("input",".price_input",function(){
    ...
    $('#item_select').on('change loaded.bs.select', function() {
...
function get_item_preview_values() {
    ...
    response.rate = $('.main input[name="rate"]').val().replace(/[^0-9\.]/g, "");;
